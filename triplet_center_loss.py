...
class TriCenLossbyPart(nn.Module):
    def __init__(self, num_classes=10, feat_dim=1024, use_gpu=True):
        super(TriCenLossbyPart, self).__init__()
    ...
    def forward(self, x, labels, margin):
        ...
        for i in range(batch_size):
            congener_dist = distmat[i][mask[i]]
            ...
            dist.append(congener_dist+max(margin-nearst_inhomogen_dist, zero))
        # print(dist)
        dist = torch.cat(dist)
        loss = dist.mean()
        return loss
